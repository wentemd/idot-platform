...
async def search_pay_item(
    ...
    query = """
        SELECT 
            ...
            contract_number,
            letting_date,
            substr(letting_date, 1, 4) as letting_year,
            county,
            bidder_name,
            bidder_rank,
            is_winner,
    ...
    if year_start:
        query += " AND CAST(substr(letting_date, 1, 4) AS INTEGER) >= ?"
    ...
    stats_query = """
        SELECT 
            substr(letting_date, 1, 4) as year,
            COUNT(*) as bid_count,
    ...
    stats_query += " GROUP BY substr(letting_date, 1, 4) ORDER BY year"
...
async def get_platform_summary():
    ...
    cursor.execute("""
        SELECT 
            MIN(substr(letting_date, 1, 4)) as min_year,
            MAX(substr(letting_date, 1, 4)) as max_year
        FROM bids
    """)
    row = cursor.fetchone()
    ...
    cursor.execute("""
        ...
        FROM bids
        GROUP BY substr(letting_date, 1, 4)
        ORDER BY year
    """)
...
async def get_top_contractors(
    ...
    if year:
        query += " WHERE substr(letting_date, 1, 4) = ?"
...
async def list_contracts(
    ...
    if year:
        query += " AND substr(letting_date, 1, 4) = ?"
        params.append(str(year))
